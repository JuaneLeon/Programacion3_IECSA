...
int main() {
	...
	while (running) {
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			// Si alguna coordenada sale de la pantalla, invertir la componente de velocidad
			bool bounced = false;
			if (minX < 0) {
				vx = fabs(vx);
				tx += (0 - minX); // corregir para que entre en pantalla
				bounced = true;
			}
			if (maxX > WIDTH) {
				vx = -fabs(vx);
			...
			// Si rebotó, opcionalmente podemos invertir la rotación para más dinamismo (comentado)
			// if (bounced) rotVel = -rotVel;

			redraw = true;
		} else if (ev.type == ALLEGRO_EVENT_KEY_DOWN) {
			// acciones puntuales: ESC para salir, R para reset
			switch (ev.keyboard.keycode) {
				case ALLEGRO_KEY_ESCAPE: running = false; break;
				case ALLEGRO_KEY_R:
					// reset a centro y velocidades por defecto
					tx = 0.0; ty = 0.0; rotAngle = 0.0;
					vx = 3.5; vy = 2.5; rotVel = 0.03;
					break;
				default: break;
			}
